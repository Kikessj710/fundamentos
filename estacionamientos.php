...
puesto.addEventListener("click", () => {
    if (puesto.classList.contains("disponible")) {  
        ...
        // Verificar si el puesto está reservado por el usuario actual
        if (puesto.dataset.usuarioId !== usuarioActualId) {
            alert("No disponible, si se desea gestionar la reserva en el apartado de mis reservas.");
        } else {
            // Si el puesto es del mismo usuario, mostrar los detalles de la reserva
            const reservaDetalles = puesto.dataset.reserva; // Suponemos que ya tienes los detalles de la reserva en el dataset
            const reservaFecha = reservaDetalles.split(" ")[0];  // Suponiendo formato: "fecha horaInicio-horaFin"
            const reservaHoras = reservaDetalles.split(" ")[1];  // El rango de horas

            // Mostrar el mensaje de la reserva
            alert(`Reserva realizada: ${reservaFecha} ${reservaHoras}\nPara cancelar la reserva, dirígete a la sección de "Mis Reservas".`);
        }
    } else {
